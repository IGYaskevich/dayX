...
export const AnimatedBackground = () => {
  ...
  return (
    <div className="pointer-events-none absolute inset-0 -z-10 overflow-hidden">
      ...
      <motion.div
        className="absolute -top-24 left-[10%] h-64 w-64 rounded-full bg-ivory-200/70 blur-3xl"
        animate={{ x: [0, 30, 0], y: [0, 20, 0], scale: [1, 1.06, 1] }}
        transition={{ duration: 16, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="absolute top-[20%] right-[5%] h-80 w-80 rounded-full bg-ivory-200/70 blur-3xl"
        animate={{ x: [0, -40, 0], y: [0, -30, 0], scale: [1, 0.96, 1] }}
        transition={{ duration: 20, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="absolute bottom-[10%] left-[20%] h-72 w-72 rounded-full bg-ivory-200/70 blur-3xl"
