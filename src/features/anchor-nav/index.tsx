        ...
        <div className="hidden md:flex items-center gap-4">
          {weddingConfig.nav.map((item) => (
            <a
              key={item.id}
              href={`#${item.id}`}
              className="text-sm text-ivory-900 hover:text-ivory-900"
            >
              {item.label}
            </a>
          ))}
          ...
          <div className="flex flex-col gap-2">
            {weddingConfig.nav.map((item) => (
              <a
                key={item.id}
                href={`#${item.id}`}
                className="text-sm text-ivory-900"
